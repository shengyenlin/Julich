...
class IRSDE(SDE):
    ...
    def noise_state_2(self, tensor):
        return tensor

    def generate_random_states_always_max(self, x0, mu, T):
        x0 = x0.to(self.device)
        mu = mu.to(self.device)
...
class DenoisingSDE(SDE):
    ...
    # since we pre add the max-level noise
    # always set to the max timestep 
    def generate_random_states_pre_add(self, x0, T):
        x0 = x0.to(self.device)
