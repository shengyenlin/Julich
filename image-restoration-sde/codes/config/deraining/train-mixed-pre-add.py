...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            LQ, GT = train_data["LQ"], train_data["GT"] # rician, clean
            timesteps, states = sde.generate_random_states_always_max(
                x0=GT, 
                mu=LQ,
                T=opt["sde"]["max_sigma"]
                )

            model.feed_data(states, LQ, GT) # xt = mixed (rician + gaussian), mu (rician), x0 (clean)
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for i, val_data in enumerate(val_loader):
                    ...
                    ##### Difference, denoising-sde doesn't have this part
                    # will add gaussian noise = 50 here
                    noisy_state = sde.noise_state(LQ)
                    #######################################################

                    # valid Predictor
                    model.feed_data(
                        noisy_state,  # xt = mixed (rician + gaussian)
                        LQ, # mu (rician)
                        GT # x0 (clean)
                        )
                    model.test(sde)
                    ...
                    print(f"Finish inference {i+1} val img, {time_elapsed} mins elapsed")
