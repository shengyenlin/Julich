...
for dataset in datasets:
    ...
    for sigma_test in sigmas:
        file_path = os.path.join(
            'results', args.pred_dir, 
            args.model_type, dataset, str(sigma_test)
            )
        ...
        print('For {:s} dataset - Noise Level {:d} -'.format(dataset, sigma_test), end = ' ')
        print('PSNR: {:f} - SSIM: {:f}'.format(avg_psnr, avg_ssim))
