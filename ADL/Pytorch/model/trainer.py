...
class Trainer(abc.ABC):
    def __init__(self,
        ...
        self.test_pic_path = self.ckpts_path.replace("checkpoints", "logs/test_pics")
        self.val_pic_path = self.ckpts_path.replace("checkpoints", "logs/val_pics")
        os.makedirs(self.test_pic_path, exist_ok=True)
        os.makedirs(self.val_pic_path, exist_ok=True)
            ...
            for i, batch_i in enumerate(train_data):
                ...
                if  (step_i>0) and (step_i%validation_per_step == 0):
                    if  len(validation_data)>0:
                        summary_val = self._get_val(self.model, self.mode, validation_data, step_i)
                ...
                if (step_i>0) and (step_i%test_per_step == 0) and (len(test_data)>0):
                    summary_test = self._get_test(self.model, self.mode, test_data, step_i)
                    if  self.rank == 0:
    ...
    def _get_train_one_batch(self, data, step_i, data_type=None):
        ...
        # get prediction
        y = data['y'].to(torch.float32).cuda(self.device) # LQ
        y_pred, y_pred_x2, y_pred_x4 = self.model(y)


        # apply evaluation metrics
        summary = {}
        # l1, historgram, pyramidal textural
    ...
    def _get_val(self, model, mode, validation_data, step_i):
        model.eval()
        ...
        with torch.no_grad():
            ...
            if self.rank == 0 and SAVE_IMGS:
                ...
                save_image(y_pred, f'{self.val_pic_path}/val_{step_i}_.png')
    ...
    def _get_test(self, model, mode, test_data, step_i):
        model.eval()
        ...
        with torch.no_grad():
            ...
            if self.rank==0 and SAVE_IMGS:
                ...
                save_image(y_pred, f'{self.test_pic_path}/test_{step_i}_.png')
